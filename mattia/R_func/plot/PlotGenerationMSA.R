...
plot.generation = function(trace.fname){
  
  trace=as.data.frame(read.csv(trace.fname, sep = ""))
  ...
  BIGHIST<- ggplot(data=Trace_big_hist,
    ...
    geom_vline(xintercept=as.Date("2020-03-02"), colour="#47606c", linetype = "longdash" )+
    annotate("text",x=as.Date("2020-03-03"), label="\n\n\nmandatory surgical mask", y= Inf, colour="#47606c",size=6,hjust = 0) +
  ...
  return(list(Infecttion = list(Comulative =HIST, AgeDivision =BIGHIST ),
              Death  = list(Comulative = HIST_d, AgeDivision = BIGHIST_d)))
}