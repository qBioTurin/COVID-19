...
List_dati<-lapply(1:length(folders), function(i){
  ...
  data<-lapply(1:length(ldeath), function(ii){
    ldeath[[ii]]->ltemp
  ...
  DeathGG$Ages =rep(AgesNames[-1],each=length(unique(trace$Time)))
  
  DeathGG$PercMask = Scenari$percMask[i]
  DeathGG$detection = Scenari$detection[i]
  
  return(DeathGG)
})


DeathGG<-do.call("rbind",List_dati)
...
pl1<-ggplot(data=DeathGG[which(DeathGG$Ages == "Comulative"),],
  ...
  scale_x_date( breaks = seq(as.Date("2020/02/21"), length.out = length(unique(DeathGG$Time)), by = "2 week"), date_labels = "%b-%d")+
...
z <- gtable_add_grob(z, 
                     list(rectGrob(gp = gpar(col = NA, fill = "gray85", size = .5)),
                          textGrob("Decreasing of the contact (mask, etc)", gp = gpar(cex = 1.8, fontface = 'bold', col = "black"))), 
                     t=7, l=5, b=7, r=11, name = c("a", "b"))

z <- gtable_add_grob(z, 
                     list(rectGrob(gp = gpar(col = NA, fill = "gray85", size = .5)),
                          textGrob("Detection rate from 2020-05-18", rot = -90,  
...
ggsave(plot = z,filename = "Plot1_Death.pdf",
...
ggsave(plot = z,filename = "Plot2_Death.pdf",
       dpi = 400, width = 40, height = 16,device = "pdf")

##### Plot 3 ( witohou 0,0 but fixed scale)


DeathGGFiltered <- DeathGG[which(DeathGG$detection != "0" | DeathGG$PercMask != "0"), ]

pl3<-ggplot(data=DeathGGFiltered[which(DeathGGFiltered$Ages == "Comulative"),],
            aes(x=Time,y=D,fill="Deaths") )+
  geom_bar(stat="identity",alpha=.5)+
  geom_line(data=reference_big_hist_DEATH[which(reference_big_hist_DEATH$Ages == "Comulative"),],
            aes(x=Time,y=I,col="red"),size=1.1)+
  ...
  geom_vline(xintercept=as.Date("2020-06-12"), colour="#47606c", linetype = "longdash" )+
  annotate("text",x=as.Date("2020-06-13"), label="\n\n\n\n\n Third release", y= Inf, colour="#47606c", size=6,hjust = 0)+
  scale_x_date( breaks = seq(as.Date("2020/02/21"), length.out = length(unique(DeathGGFiltered$Time)), by = "2 week"), date_labels = "%b-%d")+
  scale_y_continuous(breaks= scales::pretty_breaks(n = 8))
...
ggsave(plot = z,filename = "Plot3_Death.pdf",
       dpi = 400, width = 40, height = 16,device = "pdf")



##### Plot 3 ( only 0,0 but fixed scale)


DeathGGFiltered <- DeathGG[which(DeathGG$detection == "0" & DeathGG$PercMask == "0"), ]

pl3.1<-ggplot(data=DeathGGFiltered[which(DeathGGFiltered$Ages == "Comulative"),],
              aes(x=Time,y=D,fill="Deaths") )+
  geom_bar(stat="identity",alpha=.5)+
  geom_line(data=reference_big_hist_DEATH[which(reference_big_hist_DEATH$Ages == "Comulative"),],
            aes(x=Time,y=I,col="red"),size=1.1)+
...
ggsave(plot = pl3.1,filename = "Plot4_Death.pdf",
